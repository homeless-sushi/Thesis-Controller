...
OUT_DIR = os.path.join(PROJECT_DIR,"schedules/cutcp_interference/")
...
def main() :
    ...
    for n1, _, target1 in data :
        for n2, _, target2 in data :
            ...
            if n1 != n2 :
                instance1_name = f"CUTCP_{n1}"
                instance2_name = f"CUTCP_{n2}"
            else :
                instance1_name = "FIRST"
                instance2_name = "SECOND"

            file_url = os.path.join(OUT_DIR, f"cutcp_{n1}_{n2}.csv")
            with open(file_url, "w") as schedule_file :
                ...
                schedule_file.write(f"CUTCP,{instance1_name},0,{n1}.txt,FIRST.txt,{target1},\n")
                schedule_file.write(f"CUTCP,{instance2_name},0,{n2}.txt,SECOND.txt,{target2},\n")
    ...
    for n, _, target in data :
        ...
        file_url = os.path.join(OUT_DIR, f"cutcp_{n}.csv")
        with open(file_url, "w") as schedule_file :
            instance_name = f"CUTCP_{n}"
            schedule_file.write("BENCHMARK,INSTANCE_NAME,START_TIME,INPUT,OUTPUT,THROUGHPUT,APPEND\n")
            schedule_file.write(f"CUTCP,{instance_name},0,{n}.txt,FIRST.txt,{target},\n")
