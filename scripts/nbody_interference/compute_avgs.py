...
IN_DATA_DIR = os.path.join(PROJECT_DIR,"data/nbody_interference")
...
def main() :
    ...
    solo = []
    for n, p, _ in data :
        df = pd.read_csv(os.path.join(IN_DATA_DIR,f"nbody_{n}/controller.csv"))
        df = df[df["CURRENT"] != 0]
    ...
    for n1, p1, _ in data :
        for n2, p2, _ in data :
            df = pd.read_csv(os.path.join(IN_DATA_DIR,f"nbody_{n1}_{n2}/controller.csv"))

            instance_name = f"NBODY_{n1}"
            if n1 == n2 :
                instance_name = "FIRST"
